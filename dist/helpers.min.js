 ...
 * @return {Boolean}
 * @example https://any-domain.kz -> true
 */exports.isKZ=isKZ;/** Работа с Cookie */var cookie=Object.freeze({/**
   ...
   */set:function set(name,value,options){if(!name){return}var pair=[name,value].filter(Boolean).map(function(string){return encodeURIComponent(string.trim())}).join("=");var params=cookie.createOptions(options);document.cookie=[pair,params].join("; ")},/**
   * Удаление Cookie по ключу
   * @method delete
   * @param {String} name
   */delete:function _delete(name){var value=cookie.get(name)?"<removed>":"";cookie.set(name,value,{"Max-Age":-1})}});/** Работа с Cookie */exports.cookie=cookie;/**
 ...
 */var toISODate=function toISODate(date){if(!dateIsValid(date)){return""}var options={year:"numeric",month:"2-digit",day:"2-digit"};return new Intl.DateTimeFormat("ru-RU",options).format(date).split(".").reverse().join("-")};/**
 * Преобразование даты в ISO формат
 * @function toISODate
 * @param {Date} date
 * @return {String}
 ...
 */var ISOToDateFormat=function ISOToDateFormat(ISODate){var timeZone=arguments.length>1&&arguments[1]!==undefined?arguments[1]:"Europe/Moscow";var datePattern=/^(\d{4})-(\d{1,2})-(\d{1,2})$/;// 'YYYY-MM-DD'
 ...
 */var minutesToHoursMinutes=function minutesToHoursMinutes(value){var type=getType(value);var invalid=!value||type!=="Number";var number=invalid?0:value;var absNumber=Math.abs(number);var sign=number<0?"-":"";var hours=leadingZero(Math.floor(absNumber/60));var minutes=leadingZero(absNumber%60);var time=[hours,minutes].join(":");return[sign,time].join("")};/**
 * Преобразование числа в формат HH:MM
 * @function minutesToHoursMinutes
 * @param {Number} value
 * @return {String}
 ...
 * @example
 * weekNumberToDate(2020, 43); // Wed Oct 21 2020 00:00:00 GMT+0300
 */exports.weekNumberToDate=weekNumberToDate;/** Форматирование по маске */var maskIt=Object.freeze({/** Набор спец.символов */special:/[\s-+/._]/,/** Набор статических символов, без замены */static:/[7x]/,/** Словарь для составления регулярных выражений */dictionary:{Z:"[A-Z]",9:"\\d",7:"7",x:"x"," ":"\\s","+":"\\+","-":"-",_:"_","/":"/",".":"\\."},/**
   * Очистка маски
   * @method clear
   * @param {String} value
   * @return {String}
   ...
   */clear:function clear(value){if(!value){return""}var regExp=new RegExp(maskIt.special,"g");return String(value).replace(regExp,"")},/**
   ...
   */format:function format(mask,value){var clearValue=maskIt.clear(value);if(!mask||!clearValue){return""}var maskArray=mask.split("");var count=0;var formatValue="";var _iterator=_createForOfIteratorHelper(maskArray),_step;try{for(_iterator.s();!(_step=_iterator.n()).done;){var symbol=_step.value;if(count<clearValue.length){var isSpecialCharacter=maskIt.special.test(symbol);if(isSpecialCharacter){formatValue+=symbol}else{var isStaticCharacter=maskIt.static.test(symbol);formatValue+=isStaticCharacter?symbol:clearValue.charAt(count);count+=1}}else{break}}}catch(err){_iterator.e(err)}finally{_iterator.f()}return formatValue},/**
   * Создание регулярного выражения по маске
   * @method createRegExpByMask
   * @param {String} [mask]
   * @return {RegExp}
   */createRegExpByMask:function createRegExpByMask(mask){var fullValue=/.*/;if(!mask){return fullValue}var regExp="";var addRegExpRange=function addRegExpRange(string){if(!string){return}var symbol=string.charAt(0);if(maskIt.dictionary[symbol]){var length=string.match(new RegExp("".concat(maskIt.dictionary[symbol],"*")))[0].length;regExp+="".concat(maskIt.dictionary[symbol],"{").concat(length,"}");addRegExpRange(string.slice(length))}else{addRegExpRange(string.slice(1))}};addRegExpRange(mask);return regExp?new RegExp(regExp,"i"):fullValue},/**
   ...
   */check:function check(mask,value){var regExp=maskIt.createRegExpByMask(mask);var formatValue=maskIt.format(mask,value);return regExp.test(formatValue)}});/** Форматирование по маске */exports.maskIt=maskIt;/**
 * Окончания слов
 ...
 */var convertFileToBase64=function convertFileToBase64(file){if(!file){return Promise.resolve("")}var reader=new FileReader;return new Promise(function(resolve){reader.onload=function(){resolve(reader.result.split("base64,",2)[1])};reader.readAsDataURL(file)})};/**
 * Преобразование файла в Base64
 * @function convertFileToBase64
 * @param {File} file
 * @return {Promise<String>}
 ...
 */var removeObjectKeys=function removeObjectKeys(exclusionFields,sourceObject){var object=_objectSpread({},sourceObject||{});(exclusionFields||[]).forEach(function(key){delete object[key]});return object};/**
 * Удаление ключей из объекта
 * @function removeObjectKeys
 * @param {Array} exclusionFields
 * @param {Object} sourceObject
 ...
 */var memo=function memo(callback){return new Proxy(callback,{cache:new Map,apply:function apply(fn,context,args){args.sort();var key=args.toString();if(!this.cache.has(key)){this.cache.set(key,fn.apply(context,args))}return this.cache.get(key)}})};/**
 * Мемоизация
 * @function memo
 * @param {Function} callback
 * @return {Function}
