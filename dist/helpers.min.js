 ...
 */exports.isKZ=isKZ;/** Работа с Cookie */var cookie=Object.freeze({/**
   ...
   * @method delete
   * @param {String} name
   */delete:function _delete(name){var value=this.get(name)?"<removed>":"";this.set(name,value,{"Max-Age":-1})}});/** Работа с Cookie */exports.cookie=cookie;/**
 ...
 * @example
 * weekNumberToDate(2020, 43); // Wed Oct 21 2020 00:00:00 GMT+0300
 */exports.weekNumberToDate=weekNumberToDate;/** Форматирование по маске */var maskIt=Object.freeze({/**
   * Набор спец.символов
   * @type RegExp
   */special:/[\s-+/._]/,/**
   ...
   * @type Object
   */dictionary:Object.freeze({Z:"[A-Z]",9:"\\d",7:"7",x:"x"," ":"\\s","+":"\\+","-":"-",_:"_","/":"/",".":"\\."}),/**
   * Очистка маски
   * @method clear
   * @param {String} value
   * @return {String}
   ...
   */clear:function clear(value){if(!value){return""}var regExp=new RegExp(this.special,"g");return String(value).replace(regExp,"")},/**
   ...
   */format:function format(mask,value){var clearValue=this.clear(value);if(!mask||!clearValue){return""}var maskArray=mask.split("");var count=0;var formatValue="";var _iterator=_createForOfIteratorHelper(maskArray),_step;try{for(_iterator.s();!(_step=_iterator.n()).done;){var symbol=_step.value;if(count<clearValue.length){var isSpecialCharacter=this.special.test(symbol);if(isSpecialCharacter){formatValue+=symbol}else{var isStaticCharacter=this.static.test(symbol);formatValue+=isStaticCharacter?symbol:clearValue.charAt(count);count+=1}}else{break}}}catch(err){_iterator.e(err)}finally{_iterator.f()}return formatValue},/**
   * Создание регулярного выражения по маске
   * @method createRegExpByMask
   * @param {String} [mask]
   * @return {RegExp}
   */createRegExpByMask:function createRegExpByMask(mask){var _this=this;var fullValue=/.*/;if(!mask){return fullValue}var regExp="";var addRegExpRange=function addRegExpRange(string){if(!string){return}var symbol=string.charAt(0);if(_this.dictionary[symbol]){var length=string.match(new RegExp("".concat(_this.dictionary[symbol],"*")))[0].length;regExp+="".concat(_this.dictionary[symbol],"{").concat(length,"}");addRegExpRange(string.slice(length))}else{addRegExpRange(string.slice(1))}};addRegExpRange(mask);return regExp?new RegExp(regExp,"i"):fullValue},/**
   ...
   */check:function check(mask,value){var regExp=this.createRegExpByMask(mask);var formatValue=this.format(mask,value);return regExp.test(formatValue)}});/** Форматирование по маске */exports.maskIt=maskIt;/**
 * Окончания слов
